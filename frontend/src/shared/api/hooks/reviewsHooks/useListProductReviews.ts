...
export type ListProductReviewsQueryKey = ReturnType<
  typeof listProductReviewsQueryKey
>;
...
>(
  ...
  const queryKey =
    ...
    listProductReviewsQueryKey({ product_id }, params);

  const query = useQuery(
    {
      ...(listProductReviewsQueryOptions(
        { product_id },
        params,
        config
    ...
    queryClient
  ) as UseQueryResult<TData, ResponseErrorConfig<ListProductReviews422>> & {
    queryKey: TQueryKey;
  };
