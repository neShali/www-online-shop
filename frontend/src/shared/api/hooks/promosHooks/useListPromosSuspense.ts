 ...
 * {@link /api/v1/promos/}
 */
export async function listPromosSuspense(
...
export function listPromosSuspenseQueryOptions(
  params?: ListPromosQueryParams,
  config: Partial<RequestConfig> & { client?: typeof client } = {}
) {
 ...
 * {@link /api/v1/promos/}
 */
export function useListPromosSuspense<
  TData = ListPromosQueryResponse,
  TQueryKey extends QueryKey = ListPromosSuspenseQueryKey,
>(
  params?: ListPromosQueryParams,
  options: {
    query?: Partial<
  ...
  const query = useSuspenseQuery(
    {
      ...(listPromosSuspenseQueryOptions(
        params,
        config
    ...
    queryClient
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<ListPromos422>> & {
    queryKey: TQueryKey;
  };
