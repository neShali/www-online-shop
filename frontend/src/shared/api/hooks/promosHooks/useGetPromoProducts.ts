...
export const getPromoProductsQueryKey = ({
  promo_id,
}: {
...
export type GetPromoProductsQueryKey = ReturnType<
  typeof getPromoProductsQueryKey
>;
...
>(
  ...
  const queryKey =
    queryOptions?.queryKey ?? getPromoProductsQueryKey({ promo_id });

  const query = useQuery(
    {
      ...(getPromoProductsQueryOptions(
        { promo_id },
        config
    ...
    queryClient
  ) as UseQueryResult<TData, ResponseErrorConfig<GetPromoProducts422>> & {
    queryKey: TQueryKey;
  };
