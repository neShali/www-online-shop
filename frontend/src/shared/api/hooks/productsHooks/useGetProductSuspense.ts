...
>(
  ...
  const queryKey =
    queryOptions?.queryKey ?? getProductSuspenseQueryKey({ product_id });

  const query = useSuspenseQuery(
    {
      ...(getProductSuspenseQueryOptions(
        { product_id },
        config
    ...
    queryClient
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<GetProduct422>> & {
    queryKey: TQueryKey;
  };
