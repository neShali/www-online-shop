 ...
 */
export async function listCategoriesSuspense(
  params?: ListCategoriesQueryParams,
...
>(
  ...
  const queryKey =
    queryOptions?.queryKey ?? listCategoriesSuspenseQueryKey(params);

  const query = useSuspenseQuery(
    {
      ...(listCategoriesSuspenseQueryOptions(
        params,
        config
    ...
    queryClient
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<ListCategories422>> & {
    queryKey: TQueryKey;
  };
