...
export const getProductQueryKey = ({ product_id }: { product_id: GetProductPathParams['product_id'] }) =>
 ...
 * {@link /api/v1/products/:product_id}
 */
export function useGetProduct<TData = GetProductQueryResponse, TQueryData = GetProductQueryResponse, TQueryKey extends QueryKey = GetProductQueryKey>(
  { product_id }: { product_id: GetProductPathParams['product_id'] },
  options: {
    query?: Partial<QueryObserverOptions<GetProductQueryResponse, ResponseErrorConfig<GetProduct422>, TData, TQueryData, TQueryKey>> & { client?: QueryClient }
  ...
  const query = useQuery(
    {
      ...(getProductQueryOptions({ product_id }, config) as unknown as QueryObserverOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<QueryObserverOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<GetProduct422>> & { queryKey: TQueryKey }
