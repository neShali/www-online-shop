...
export type ListProductReviewsSuspenseQueryKey = ReturnType<typeof listProductReviewsSuspenseQueryKey>
 ...
 * {@link /api/v1/reviews/product/:product_id}
 */
export function useListProductReviewsSuspense<TData = ListProductReviewsQueryResponse, TQueryKey extends QueryKey = ListProductReviewsSuspenseQueryKey>(
  { product_id }: { product_id: ListProductReviewsPathParams['product_id'] },
  params?: ListProductReviewsQueryParams,
  options: {
    query?: Partial<UseSuspenseQueryOptions<ListProductReviewsQueryResponse, ResponseErrorConfig<ListProductReviews422>, TData, TQueryKey>> & {
      client?: QueryClient
    }
  ...
  const queryKey = queryOptions?.queryKey ?? listProductReviewsSuspenseQueryKey({ product_id }, params)

  const query = useSuspenseQuery(
    {
      ...(listProductReviewsSuspenseQueryOptions({ product_id }, params, config) as unknown as UseSuspenseQueryOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<UseSuspenseQueryOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<ListProductReviews422>> & { queryKey: TQueryKey }
