...
export const getPromoQueryKey = ({ promo_id }: { promo_id: GetPromoPathParams['promo_id'] }) =>
 ...
 */
export async function getPromo({ promo_id }: { promo_id: GetPromoPathParams['promo_id'] }, config: Partial<RequestConfig> & { client?: typeof client } = {}) {
 ...
 * {@link /api/v1/promos/:promo_id}
 */
export function useGetPromo<TData = GetPromoQueryResponse, TQueryData = GetPromoQueryResponse, TQueryKey extends QueryKey = GetPromoQueryKey>(
  { promo_id }: { promo_id: GetPromoPathParams['promo_id'] },
  options: {
    query?: Partial<QueryObserverOptions<GetPromoQueryResponse, ResponseErrorConfig<GetPromo422>, TData, TQueryData, TQueryKey>> & { client?: QueryClient }
  ...
  const query = useQuery(
    {
      ...(getPromoQueryOptions({ promo_id }, config) as unknown as QueryObserverOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<QueryObserverOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseQueryResult<TData, ResponseErrorConfig<GetPromo422>> & { queryKey: TQueryKey }
