...
export const getPromoProductsSuspenseQueryKey = ({ promo_id }: { promo_id: GetPromoProductsPathParams['promo_id'] }) =>
  [{ url: '/api/v1/promos/:promo_id/products', params: { promo_id: promo_id } }] as const

export type GetPromoProductsSuspenseQueryKey = ReturnType<typeof getPromoProductsSuspenseQueryKey>
 ...
 * {@link /api/v1/promos/:promo_id/products}
 */
export function useGetPromoProductsSuspense<TData = GetPromoProductsQueryResponse, TQueryKey extends QueryKey = GetPromoProductsSuspenseQueryKey>(
  { promo_id }: { promo_id: GetPromoProductsPathParams['promo_id'] },
  options: {
    query?: Partial<UseSuspenseQueryOptions<GetPromoProductsQueryResponse, ResponseErrorConfig<GetPromoProducts422>, TData, TQueryKey>> & {
      client?: QueryClient
    }
  ...
  const queryKey = queryOptions?.queryKey ?? getPromoProductsSuspenseQueryKey({ promo_id })

  const query = useSuspenseQuery(
    {
      ...(getPromoProductsSuspenseQueryOptions({ promo_id }, config) as unknown as UseSuspenseQueryOptions),
      queryKey,
      ...(queryOptions as unknown as Omit<UseSuspenseQueryOptions, 'queryKey'>),
    },
    queryClient,
  ) as UseSuspenseQueryResult<TData, ResponseErrorConfig<GetPromoProducts422>> & { queryKey: TQueryKey }
